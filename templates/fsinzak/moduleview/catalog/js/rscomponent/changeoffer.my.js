...
class ProductChangeOffer
{
    ...
    changePrice(selectedOfferData) {
        // Заменяет data-href у кнопки
        let replacer = (element) => {
            let queryParams = 'offer_id=' + selectedOfferData.id;
            element.dataset['href'] = element.dataset['baseHref'] + "&" + queryParams;
        };
        document.querySelectorAll('.rs-first-add-to-cart').forEach(replacer);
        ...
        if (selectedOfferData.price) {
            ...
            $('.offer-stock').text(parseInt(selectedOfferData.num));
        }
