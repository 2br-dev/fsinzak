        ...
        <div class="variant-product-options__list mb-4">
            {foreach $offers_data['levels'] as $level}
                <div class="sets-wrapper">
                    <div class="sets-title">{$level.title}:</div>
                    <ul class="item-product-choose sets">
                        {foreach $level.values as $key => $value}
                            <li>
                                <div class="radio-{if isset($value['image'])}image{else}{$level.type|default:"list"}{/if} set">
