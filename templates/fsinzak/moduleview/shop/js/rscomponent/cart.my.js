...
new class Cart extends RsJsCore.classes.component {
    ...
    requestToCart(url, formData, noShowModalCart) {
        this.utils.fetchJSON(url, {
            ...
            // OpenDialog.show();
            if(response.cart_error.length > 0){
                $('#product-button-'+response.id+'').removeClass('item-product-cart-action_amount');
                if(response.cart_error[0] === 'limit_weight'){
                    M.toast({html: '<p>Превышен лимит веса в заказе. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                }
                if(response.cart_error[0] === 'limit_sum'){
                    M.toast({html: '<p>Превышен лимит суммы заказа. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                }
                if(response.cart_error[0] === 'periodicity'){
                    M.toast({html: '<p>Превышен лимит колчиество заказов. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                }
            }else {
    ...
    refresh(url, callback) {
        ...
        this.utils.fetchJSON(url, {
            ...
            if(response.limit_error !== ""){
                ...
                if(response.limit_error === 'sum'){
                    M.toast({html: '<p>Превышен лимит общей суммы заказа. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                }
                if(response.limit_error === 'product'){
                    M.toast({html: '<p>Превышен лимит этого товара в корзине. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                }
            }
        ...
        if (!event.detail || !event.detail.noRefreshCart) {
            ...
            RsJsCore.utils.fetchJSON(this.smartButton.dataset.url, {
                ...
                if (response.success) {
                    ...
                    if(response.cart_error.length > 0){
                        this.amountInput.value = response.amount;
                        if(response.cart_error[0] === 'limit_weight'){
                            M.toast({html: '<p>Превышен лимит веса в заказе. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                        }
                        if(response.cart_error[0] === 'limit_sum'){
                        ...
                        if(response.cart_error[0] === 'periodicity'){
                            M.toast({html: '<p>Превышен лимит колчиества заказов. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                        }
                        if(response.cart_error[0] === 'limit_product_amount'){
                            M.toast({html: '<p>Превышен лимит данного товара в корзине. Товар не добавлен в корзину</p>', classes: 'toast-error'});
                        }
                    }
                }
            });
