...
{*                </div>*}
{*            {/if}*}
            <form class="rs-checkout_form row" method="POST">
                <div class="rs-checkout_totalBlock col s12">{$blocks.total}</div>
                {$blocks.user}
...
{*                    {/if}*}
                ...
                <div class="col l4 m12 s12 rs-checkout_commentBlock">{$blocks.comment}</div>
{*                </div>*}
                <div class="col s12 right-align">
                    <button type="submit" class="btn">{t}Оформить заказ{/t}</button>
                </div>
            </form>
        </div>
