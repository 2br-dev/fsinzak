{$user = $order->getUser()}
<main id="order-proceed">
    <section>
        <div class="container">
            <div class="row flex">
                <div class="col s12">
                    <h1>{t number=$order.order_num}Заказ №%number успешно оформлен{/t}</h1>
                </div>
                <div class="col m8 s12 order-m1">
                    <div class="info-wrapper">
                        <div class="icon-wrapper">
                            <i class="mdi mdi-check success"></i>
                        </div>
                        <div class="data-wrapper">
                            <div class="success-header">Готово!</div>
                            <p>Мы свяжемся с Вами в ближайшее время. Ваш заказ будет доступен для просмотра и редактирования из секции <a href="/orders.html">«Мои заказы»</a> на странице профиля </p>
                        </div>
                    </div>
                    <p>Заказ мы также продублируем на Ваш e-mail. Вы можете закрыть эту страницу, или <a href="/">продолжить покупки</a></p>
                </div>
                <div class="col m4 s12">
                    <div class="details">
                        <div class="details-header">Состав заказа</div>
                        <div class="kv-table" id="details">
                            ...
                            {foreach $orderdata.items as $n=>$item}
                                ...
                                <div class="kv-pair">
                                    <div class="key">{$orderitem.title}</div>
                                    <div class="value">{$item.total}</div>
                                </div>
                            {/foreach}
                        </div>
                        <div class="kv-table" id="summary">
                            <div class="kv-pair order-finish-total">
                                <div class="key">Итого</div>
                                <div class="value">{$orderdata.total_cost}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col s12 right-align">
                    {if $order->canOnlinePay()}
                        <a href="{$order->getOnlinePayUrl()}" class="btn">{t}Перейти к оплате{/t}</a>
                    {else}
                        <a href="{$router->getRootUrl()}" class="btn">{t}Завершить заказ{/t}</a>
                    {/if}
                </div>
            </div>
        </div>
