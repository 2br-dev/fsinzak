    ...
    */
    function actionIndex()
    {
        ...
        */
        ...
        $stocks = $item->getWarehouseStock();
        $affiliate = \Affiliate\Model\AffiliateApi::getCurrentAffiliate();
        $affiliate_warehouse = $affiliate->getAffiliateWarehouse();
        $item->setWarehousesForDynamicNum([$affiliate_warehouse]);
        $item->fillAffiliateDynamicNum();
//        echo '<pre>';
//        var_dump(intval($stocks[$affiliate_warehouse][0]['stock']));
//        var_dump(intval($item['dynamic_num']));
//        exit();
        if (!$item){
