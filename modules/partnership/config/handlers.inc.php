...
class Handlers extends HandlerAbstract
{
    ...
    public static function applyRoute()
    {
        ...
        if ($param_dont_redirect || $param_dont_confirm) {
            ...
            $application->redirect($request->replaceKey([
    ...
    {
        $routes[] = new Route('partnership-front-profile', [
            '/profile-partner/',
    ...
    public static function start()
    {
        ...
        if ($config['installed']) {
            ...
            if ($partner) {
                ...
                if ($partner['logo_sm']) {
                    $site['logo_sm'] = $partner['logo_sm'];
                }
                if ($partner['logo_xs']) {
                    $site['logo_xs'] = $partner['logo_xs'];
                }
                if ($partner['slogan']) {
    ...
    public static function controllerExecCatalogAdminOneClickCtrlIndex($helper)
    {
        ...
        $table->addColumn(new TableType\Userfunc('partner_id', t('Партнерский сайт'), function ($partner_id) use ($partners_list) {
            ...
            return "";
        }, ['Sortable' => SORTABLE_BOTH, 'CurrentSort' => SORTABLE_ASC]), -1);

        $container = $helper['filter']->getContainer();
        //Добавим партнерский сайт, если он присутствует
        $partners_list = ['' => t('-Не выбрано-')] + Api::staticSelectList(); //Список потрёрских сайтов в системе
        $container->addLine(new Filter\Line([
            'Items' => [
    ...
    public static function ormBeforeWriteShopOrder($params)
    {
        ...
        if ($partner) {
            $order = $params['orm'];
            $order->addExtraInfoLine(t('Оформлен от партнера'), $partner['title'], [
    ...
    {
        $orm_order->getPropertyIterator()->append([
            'partner_id' => new OrmType\Integer([
    ...
    public static function ormInitShopDelivery(Delivery $orm)
    {
        $orm->getPropertyIterator()->append([
            t('Дополнительные условия показа'),
                'show_on_partners' => new OrmType\ArrayList([
                    ...
                    'list' => [['\Partnership\Model\Api', 'staticSelectList'], [0 => t('Основной сайт')]],
                    'attr' => [[
                        'multiple' => true,
                        'size' => 5,
                    ]],
                ]),
                '_show_on_partners' => new OrmType\Varchar([
    ...
    {
        $orm_reserve->getPropertyIterator()->append([
            'partner_id' => new OrmType\Integer([
                'maxLength' => '11',
    ...
    {
        $orm_one_click->getPropertyIterator()->append([
            'partner_id' => new OrmType\Integer([
                'maxLength' => '11',
                'description' => t('Партнёрский сайт'),
                'default' => 0,
    ...
    public static function ormInitMenuMenu(Menu $menu)
    {
        ...
        $menu->getPropertyIterator()->append([
            ...
            'partner_id' => new OrmType\Integer([
                ...
                'allowEmpty' => false,
                'default' => 0,
                'listFromArray' => [$default_list + Api::staticSelectList()],
    ...
    {
        $orm_transaction->getPropertyIterator()->append([
            'partner_id' => new OrmType\Integer([
                'maxLength' => '11',
                'description' => t('Партнёрский сайт'),
                'default' => 0,
    ...
    public static function themeGetContextList($contexts)
    {
        ...
        if ($config['installed']) {
            ...
            foreach ($api->getList() as $partner) {
                ...
                $contexts[$partner->getThemeContext()] = [
                    'title' => $partner['title'],
    ...
    {
        $partner = Api::getCurrentPartner();
        $partner_id = (!empty($partner['id'])) ? $partner['id'] : '-1';
