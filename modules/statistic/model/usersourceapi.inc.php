...
class UserSourceApi
{
    ...
    public static function getLastSourceUtmCookieArray()
    {
        ...
        $data_filtered = array_intersect_key($data, array_flip([
            'utm_source',
            'utm_medium',
            'utm_campaign',
            'utm_term',
    ...
    public static function getCookieSourceInfo()
    {
        ...
        $user_source = $user_source_string ? (array)@unserialize($user_source_string) : [];

        //Оставляем только нужные ключи
        $data_filtered = array_intersect_key($user_source, array_flip([
            'partner_id',
            'referer_site',
            'referer_source',
            'landing_page',
