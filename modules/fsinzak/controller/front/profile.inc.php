...
class Profile extends AuthorizedFront
{
    ...
    public function actionIndex()
    {
        ...
        if ($this->isMyPost() && $this->url->checkCsrf()) {
            if($_POST['data']['citizen'] == 'Не гражданин РФ'){
                $user = $this->getUserForProfile(true);
            }
        ...
        return $this->result->setTemplate('%users%/profile.tpl');
    }

    public function actionCheckoutEdit()
    {
        $this->app->title->addSection(t('Профиль'));
        $this->app->breadcrumbs->addBreadCrumb(t('Профиль пользователя'));
     ...
     * @return User
     */
    protected function getUserForProfile($flag = false)
    {
