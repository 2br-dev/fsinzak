        ...
        if (isset(self::$_cache_import_hash[$xml_id]) && self::$_cache_import_hash[$xml_id] == $import_hash) {
            ...
            if (isset($this->getSimpleXML()->Цены->Цена)) {
                if ($config['dont_delete_costs']) {
                    ...
                    if ($old_pricedata) {
                        $unserialized = @unserialize($old_pricedata);
            ...
            $on_duplicate_update_fields = array_diff(['title', 'barcode', 'pricedata', 'propsdata', 'num', 'processed', 'import_hash', 'sku'], (array)$config['dont_update_offer_fields']);
