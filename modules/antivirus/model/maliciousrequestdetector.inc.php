...
class MaliciousRequestDetector extends BaseModel
{
     ...
     * @var array
     */
    private $patterns_for_user_agent = ['binlar', 'casper', 'cmswor', 'diavol', 'finder', 'flicky', 'nutch', 'planet', 'purebot', 'pycurl', 'skygrid', 'sucker', 'turnit', 'vikspi', 'zmeu'];
    ...
    private function onMaliciousRequestDetected(MaliciousRequestInfo $info)
    {
        ...
        $obj = OrmRequest::make()->from($obj)->where(['ip'=>$info->ip])->object();
    ...
    public function onMaliciousRequestCountLimitExceeded(MaliciousRequestInfo $info)
    {
        ...
        if($this->config->proactive_auto_block)
        {
            ...
            $blocked['comment'] = t('Обнаружен вредносный параметр по шаблону %0', [$info->pattern]);
            $blocked->replace();
