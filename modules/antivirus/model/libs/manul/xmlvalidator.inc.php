    ...
    private function libxmlDisplayError($error)
    {
        ...
        switch ($error->level) {
            case LIBXML_ERR_WARNING:
                $return[] = t('Предупреждение %0', [$error->code]);
                break;
            case LIBXML_ERR_ERROR:
                $return[] = t('Ошибка %0', [$error->code]);
                break;
            case LIBXML_ERR_FATAL:
                $return[] = t('Критическая ошибка %0', [$error->code]);
                break;
        }
        $return .= trim($error->message);
        if ($error->file) {
            $return[] = t('в %0', [$error->file]);
        }
        $return[] = t('в строке %0', [$error->line]);
        return join(' ', $return);
    }
