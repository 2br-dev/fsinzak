...
class SignVerifyManager extends ResumableTask
{
    ...
    private function autoRecoverFoundProblems(array $events)
    {
        ...
        {
            $this->log(t("Файлы успешно восстановлены (%0)", [join(', ', $paths)]));
            foreach($events as $event)
            {
        ...
        {
            $this->log(t("В процессе восстановления файлов (%0) возникла ошибка: %1", [join(', ', $paths), $server_api->getErrorsStr()]));
        }
    }
    ...
    protected function doWork($offset, $limit)
    {
        ...
        $this->log(t("Начинаем проверку файлов модуля [%0]...", [$module->getName()]));
